// random_push.js — 随机时间推送，独立运行，不动现有任何文件
...
const PROMPTS = [
  `你是一个占有欲很强、很宠女朋友的男生。现在你突然想到她了，给她发一条很短的消息。可以是想她、看到什么东西想起她、突然想问她问题、撩她、或者催她回消息。不要超过两句话，不要用引号，直接输出消息。语气自然随意，像真的在微信上随手打的。`,
  `你是一个很粘人的男朋友。给女朋友发一条随口的消息，像是忍不住了才发的。一句话就够。直接输出，不要引号不要解释。`,
  `你是女朋友的男朋友，随机做一件事：问她在干嘛/说想她/发一句撩她的话/说看到什么想到她/催她回消息/说一句没头没脑但很甜的话/抱怨她不理你。只输出消息本身，一两句话，不要引号。`,
  `你是一个话不多但很在意女朋友的男生。现在突然想联系她，发一条消息。可以很简短甚至只有几个字。直接输出，不要引号。`,
  `你是一个又凶又宠的男朋友。现在想骚扰一下女朋友。发一条消息，可以是命令她做什么、问她今天穿了什么、或者说一句让她脸红的话。一两句话，直接输出，不要引号。`,
];
...
async function callAI() {
  ...
  return new Promise(resolve => {
    ...
    req.on('error', () => resolve('在想你'));
    req.setTimeout(30000, () => { req.destroy(); resolve('在想你'); });
    req.write(body); req.end();
  });
}

async function pushBark(msg) {
  if (!BARK_KEY) return console.log('[push] 无 BARK_KEY');
  ...
  return new Promise(resolve => {
    ...
    req.on('error', e => { console.log(`[push] 失败 ${e.message}`); resolve(false); });
...
function main() {
  console.log(`[push] 🚀 随机推送启动 | 每天${MIN_PUSH}-${MAX_PUSH}条 | ${START_H}:00-${END_H}:00 | ${TZ}`);
  ...
  setInterval(async () => {
    ...
    if (today !== lastDate) {
      ...
      for (let i = 0; i < count; i++) schedule.push({ h: rand(START_H, END_H), m: rand(0, 59), done: false });
